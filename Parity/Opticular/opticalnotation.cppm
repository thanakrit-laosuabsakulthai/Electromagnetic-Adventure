...
export inline std::multiset<int> transcribe_numerical_dialect(std::string &numerical_dialect) {
	std::multiset<int> transcribed_numbers;
	std::istringstream dialect_stream(numerical_dialect);
	
	// clean process assume the input is space-separated numbers
	// and already validated
	
	int number;
	while (dialect_stream >> number) {
		transcribed_numbers.insert(number);
	}
