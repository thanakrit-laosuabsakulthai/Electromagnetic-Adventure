...
void Order_Of_Optics::execute(Overworld &world) {
	terra = &world;
	concentrate();
...
void Order_Of_Optics::concentrate() {
...
void Order_Of_Optics::end_concentration() {
	Overworld &world = *terra;
	world.announce.redact(); // Remove the query about purchasing from the announcement feed
}

void Order_Of_Optics::clause_invalid() {
	Overworld &world = *terra;
	world.announce.reject(); // Reject the player input
}


void Order_Of_Optics::clause_decline_shop() {
	purchasement.clear();
}

void Order_Of_Optics::apply_dialect(std::string &player_input) {
	Overworld &world = *terra;
...
void Order_Of_Optics::apply_numerical_dialect() {
	Overworld &world = *terra;

	Optics item = getOpticsFromNumber(transcribed_numerical_dialect);
	purchasement.insert(item);
}

bool Order_Of_Optics::validate_dialect(std::string &player_input) {
	Overworld &world = *terra;
...
bool Order_Of_Optics::validate_numerical_dialect() {
	Overworld &world = *terra;
...
bool Order_Of_Optics::validate_choice() {
...
void Order_Of_Optics::fill_valid_numeral() {
		Overworld &world = *terra;
...
Optics Order_Of_Optics::getOpticsFromNumber(int number) {
	Overworld &world = *terra;
